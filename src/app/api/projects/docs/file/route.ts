...
import {
  resolveLegacyPublicDocUploadPath,
  resolveManagedDocUploadPath,
...
async function resolveExistingFilePath(projectId: string, assetPath: string) {
  const candidatePaths = [
    resolveManagedDocUploadPath(projectId, assetPath),
    resolveLegacyPublicDocUploadPath(projectId, assetPath),
...
async function handleFileRequest(request: NextRequest, includeBody: boolean) {
  try {
    ...
    console.error("Read docs asset error:", error);
    return NextResponse.json(
      { error: "Erreur interne lors de la lecture du document." },
      { status: 500 }
    );
  }
}
